...
- **AI Flashcard Generator** — `FlashcardPage.tsx`, now embedded inside "Ruang Produktif" tab (Flashcard AI tab, replacing Prosedur). Two modes: topic input or paste text. `POST /api/flashcards/generate` uses GPT-4o-mini with `response_format: json_object`. Returns 3–20 Q/A pairs. Cards flip with CSS 3D animation, navigation via arrows + progress dots, shuffle button. Direct import in `ProductivityPage.tsx`.
