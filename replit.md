...
- **Productivity v2** — 5-tab system in `ProductivityPage.tsx`. Backend is layered architecture in `server/`:
  ...
  - **Flashcard AI** — AI-powered flashcard generator (moved from standalone sidebar item). Renders `FlashcardPage` component directly inside Ruang Produktif. Replaced previous Prosedur tab.
...
- **UX Improvements (Mar 2026)**:
  ...
  - Sidebar nav grouped into **Utama** (Berita, Library, Ruang Produktif) and **Komunitas** (Threads, Leaderboard, Contributor) with section labels; Admin section shown only for admins. Flashcard AI has been moved inside Ruang Produktif tab (replacing Prosedur). Jawaban Tersimpan has been moved inside Profile tab.
...
- **Library Upload Fix** — `POST /api/admin/library/upload-file` improved: MIME type fallback from filename extension (fixes blank `file.type` in some browsers), Buffer→Uint8Array for Supabase SDK compat, removed `fileSizeLimit` from bucket creation, friendly per-error messages (policy/unauthorized → 403, size limit → 413).
- **Smart Expiry Alerts (in-app + email)** — `TrackerTab` in `ProductivityPage.tsx` now shows color-coded banners: 🔴 due today/tomorrow, 🟠 due ≤7 days, 🟡 due ≤30 days (computed client-side from loaded items). Email alerts in `reminderService.js`: `sendExpiryAlert()` fires per H-1/H-7/H-30 windows with per-window anti-spam; `runExpiryAlerts()` runs all users. Wired into `GET /api/cron/daily` alongside `runDailyReminder()`.
- **AI Flashcard Generator** — `FlashcardPage.tsx`, embedded inside "Ruang Produktif" tab. Three input modes: topic / paste text / PDF upload. `POST /api/flashcards/generate` uses GPT-4o-mini. Cards flip with CSS 3D animation. Per-card progress tracking: ✓ Paham / ✗ Ulangi buttons with color-coded progress bar and dots. End-of-session summary with retry options. **Save/Load sets**: `flashcard_sets` table with RLS; `GET/POST/DELETE /api/flashcards/sets`; "Set Tersimpan" tab shows saved sets with load/delete; inline name input on save.
- **Google Maps Places Integration** — `api/engine/placesSearch.js` module. `detectPlacesQuery(text)` detects when user asks about a place/location/restaurant/clinic/bank etc (Indonesian + English patterns). `buildPlacesContext(text)` calls Google Places API (New) — `places:searchText` + `places:searchNearby` — and returns a rich context block injected into the system prompt with: name, address, phone, website, rating, opening hours (open now indicator), Google Maps link. Defaulted to Nasr City / Hay Asyir as search center (Masisir hub). `GOOGLE_MAPS_API_KEY` env secret required. Integrated into chat pipeline after PartnerPromo injection. Admin status panel now shows Google Maps as a source with trust=95.
- **Chat Export** — `exportChat()` in `ChatArea.tsx` downloads the full conversation as a `.txt` file (UTF-8) with header, timestamps, and per-message speaker labels. Export button shown in mobile header (download icon, next to Plus, only when messages exist) and as a sticky "Ekspor" button at top-right of the messages area on desktop (hidden on mobile, `md:flex`).
- **Jawaban Tersimpan inside Profile** — `SavedAnswersPage` is now embedded inside `ProfilePage.tsx` as a second tab ("Jawaban Tersimpan"), toggled via tab bar at top of Profile. Removed from standalone sidebar nav. Admin library panel now uses `GET /api/admin/library` endpoint that returns all items (including drafts) with `is_published` field; previously used public `/api/library` which only returned published items.
- **Body Limit Fix (Audio + Chat Image)** — `/api/whisper` and `/api/chat` added to `largeRoutes` (20mb limit). Previously both used default 64kb limit causing "too large" errors for voice recordings and chat image uploads.
- **KB Arabic Translation** — Admin edit dialog (`ArticleFormDialog`) now accepts `articleId` prop and shows "✨ Terjemahkan ke Arab" button. `POST /api/admin/articles/:id/translate-arabic` uses GPT-4o-mini (MSA/فصحى, temp 0.3). Result saved to `content_ar` column. Arabic content displayed RTL in dialog. `buildArticleEmbedText()` in `embedder.js` now includes `content_ar` for better Arabic semantic search. `Article` interface includes `content_ar?: string | null`.
