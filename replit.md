...
- **AI Flashcard Generator** — New `FlashcardPage.tsx` tab ("Flashcard AI" in sidebar, nav id `flashcard`, icon `Layers`). Two modes: topic input or paste text. `POST /api/flashcards/generate` uses GPT-4o-mini with `response_format: json_object`. Returns 3–20 Q/A pairs. Cards flip with CSS 3D animation, navigation via arrows + progress dots, shuffle button. Added to `VALID_TABS` in Dashboard.tsx + lazy import.
